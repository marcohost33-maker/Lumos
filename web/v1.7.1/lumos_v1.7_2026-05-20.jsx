...
// ╔══════════════════════════════════════════════════════════════════╗
// ║ LUMOS v1.7 — Nicht-dargestellt dargestellt. Mit Erinnerung.      ║
...
// ═══════════════════════════════════════════════════════════════════
function loadApiKey() {
  try { return (typeof localStorage !== 'undefined' && localStorage.getItem(STORAGE_KEY_API)) || ''; }
  catch { return ''; }
}
function saveApiKey(k) {
  try {
    if (typeof localStorage === 'undefined') return;
    if (k) localStorage.setItem(STORAGE_KEY_API, k);
    ...
    <div onMouseDown={onClose} role="dialog" aria-modal="true" aria-label="Einstellungen" style={{
      ...
      <div onMouseDown={e => e.stopPropagation()} style={{
        ...
        <div style={{
          ...
          letterSpacing: '.05em',
        }}>Lumos v1.7 — Anthropic API direkt (Streaming)</div>
      ...
      {rows.map((row, ri) => (
        <div key={ri} style={{
          ...
          {row.map((k, ki) => {
            ...
            return (
              <button key={`${ri}-${ki}`} onClick={() => onKey(k.k)}
                aria-label={
                  k.k === 'BACK' ? 'Loeschen' : k.k === 'ENTER' ? 'Senden' :
...
function LumosInner() {
  ...
  const send = useCallback(async () => {
    ...
    const key = apiKeyRef.current || loadApiKey();
    if (!key) {
  ...
  }, [appendChar, backspace, addEmitter]);
